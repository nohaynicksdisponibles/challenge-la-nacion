...
export default function Grid({gridData, subtype = 7}: IGrid){
    const {data} = useContext(provider)

    return(
        <section className="row-gap-tablet-2 row-gap-deskxl-3 hlp-degrade" data-testid="grid" style={{marginBottom: '150px'}}>
            {gridData?.filter(obj => Number(obj.subtype) === subtype).filter(obj => filterByTag(obj, data.tag)).map((data) => <Card {...data} key={data._id}/>)}
